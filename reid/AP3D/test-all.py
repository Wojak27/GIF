...
def test(model, queryloader, galleryloader, use_gpu, ranks=[1, 5, 10, 20]):
    ...
    for batch_idx, (vids, pids, camids, cloths) in enumerate(queryloader):
        if use_gpu:
            vids = vids.cuda()
        feat = model(vids)
        feat = feat.mean(1)
    ...
    # evaluate_clothes(distmat, q_pids, g_pids, q_camids, g_camids, q_oids, g_oids, mode='CC')
    ...
    # # attribute evaluation CC and SC
    # cmc_sc, mAP_sc = evaluate_clothes(distmat, q_pids, g_pids, q_camids, g_camids, q_oids, g_oids, mode='SC')
    # print("SC Results ----------")
    # print("mAP: {:.2%}".format(mAP_sc))
    # print("CMC curve")
    # for r in ranks:
    #     print("Rank-{:<3}: {:.2%}".format(r, cmc_sc[r-1]))
    # print("------------------")

    # cmc_cc, mAP_cc = evaluate_clothes(distmat, q_pids, g_pids, q_camids, g_camids, q_oids, g_oids, mode='CC')
    # print("CC Results ----------")
    # print("mAP: {:.2%}".format(mAP_cc))
    # print("CMC curve")
    # for r in ranks:
    #     print("Rank-{:<3}: {:.2%}".format(r, cmc_cc[r-1]))
    # print("------------------")
    ...
    # SC, CC and STD
    # Standard cloth evaluation (ignoring cloth labels)
    cmc_std, mAP_std = evaluate_clothes(distmat, q_pids, g_pids, q_camids, g_camids, q_oids, g_oids, mode='STD')
    print("Standard Cloth (STD) Results ----------")
    print("mAP: {:.2%}".format(mAP_std))
    print("CMC curve")
    for r in ranks:
        print("Rank-{:<3}: {:.2%}".format(r, cmc_std[r-1]))
    print("------------------")

    # SC evaluation (same clothes)
    cmc_sc, mAP_sc = evaluate_clothes(distmat, q_pids, g_pids, q_camids, g_camids, q_oids, g_oids, mode='SC')
    print("SC Results ----------")
    print("mAP: {:.2%}".format(mAP_sc))
    print("CMC curve")
    ...
    # CC evaluation (clothes-changing)
    cmc_cc, mAP_cc = evaluate_clothes(distmat, q_pids, g_pids, q_camids, g_camids, q_oids, g_oids, mode='CC')
    print("CC Results ----------")
    print("mAP: {:.2%}".format(mAP_cc))
    print("CMC curve")
