...
class MotChallenge2DBox(_BaseDataset):
    ...
    def __init__(self, config=None):
        ...
        self.gt_fol = self.gt_fol[:-1] if self.gt_fol[-1] == '/' else self.gt_fol
        ...
        for tracker in self.tracker_list:
            if self.data_is_zipped:
                ...
                for seq in self.seq_list:
                    curr_file = os.path.join(self.tracker_fol, tracker, self.tracker_sub_fol, seq.split("/")[-1] + '.txt')
                    if not os.path.isfile(curr_file):
                        print('Tracker file not found: ' + curr_file)
    ...
    def _load_raw_file(self, tracker, seq, is_gt):
        ...
        if self.data_is_zipped:
            ...
            if is_gt:
                ...
                file = os.path.join(self.tracker_fol, tracker, self.tracker_sub_fol, seq if len(seq.split("/"))==1 else seq.split("/")[-1] + '.txt')
    ...
    def get_preprocessed_seq_data(self, raw_data, cls):
        ...
        for t in range(raw_data['num_timesteps']):
            ...
            if self.config["USE_CLASS_AS_TRACKLET"] == True:
                tracker_ids = raw_data['tracker_classes'][t]
            ...
            if 'tracker_classes' not in data:
                data['tracker_classes'] = [None] * raw_data['num_timesteps']
            data['tracker_classes'][t] = np.delete(tracker_classes, to_remove_tracker, axis=0)
        ...
        if 'gt_classes' not in data:
                data['gt_classes'] = raw_data['gt_ids']
