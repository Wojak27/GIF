...
def write_results_and_category_no_score(filename, results):
    save_format = '{frame},{id},{x1},{y1},{w},{h},-1,{r1},-1\n'
    with open(filename, 'w') as f:
        for frame_id, tlwhs, track_ids, rs in results:
            for tlwh, track_id, r in zip(tlwhs, track_ids, rs):
                if track_id < 0:
                    continue
                x1, y1, w, h = tlwh
                r1 = r
                line = save_format.format(frame=frame_id, id=track_id, x1=round(x1, 1), y1=round(y1, 1), w=round(w, 1), h=round(h, 1), r1=r1)
                f.write(line)
    logger.info('save results to {}'.format(filename))
